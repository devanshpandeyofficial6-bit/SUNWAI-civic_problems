            ...
            if (res.statusCode === 200) {
              try {
                ...
                resolve({
                  ...
                  severityAssessment: parsed.severity_assessment || null,
                  costEstimate: parsed.cost_estimate || null,
                  privacyCompliance: parsed.privacy_compliance || null,
                });
              } catch (parseErr) {
...
function verifyResolutionProof(reportedBase64, resolutionBase64, category) {
  return new Promise((resolve) => {
    try {
      const url = new URL(`${AI_SERVICE_URL}/verify-resolution`);
      const payload = JSON.stringify({
        reported_image_base64: reportedBase64,
        resolution_image_base64: resolutionBase64,
        category,
      });
        ...
        (res) => {
          ...
          res.on('end', () => {
            try {
              ...
              resolve({ verified: true, fallback: true, raw: data });
            }
          });
        }
      );
      req.on('timeout', () => {
        req.destroy();
        resolve({ verified: true, fallback: true, rejection_reason: 'Proof verification timed out; flagged for manual review.' });
      });
      req.on('error', (err) => resolve({ verified: true, fallback: true, error: err.message }));
      req.write(payload);
      req.end();
    } catch (err) {
      resolve({ verified: true, fallback: true, error: err.message });
    }
  });
}

function anonymizeImage(imageBase64) {
  return new Promise((resolve) => {
    try {
      const url = new URL(`${AI_SERVICE_URL}/anonymize`);
        ...
        (res) => {
          ...
          res.on('end', () => {
            try {
              ...
              resolve({ ok: false, image_base64: imageBase64 });
            }
          });
        }
      );
      ...
      req.on('error', () => resolve({ ok: false, image_base64: imageBase64 }));
      req.write(payload);
      req.end();
    } catch (err) {
      resolve({ ok: false, image_base64: imageBase64 });
